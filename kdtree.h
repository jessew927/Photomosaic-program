...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* constructorHelper(vector<Point<Dim>>& v, int curDim, int left, int right);
    Point<Dim> quickSelect(vector<Point<Dim>>& v, int left, int right, int k, int curDim);
    int partitionHelper(vector<Point<Dim>>& v, int left, int right, int curDim);
    void swap(Point<Dim>& x, Point<Dim>& y);
    void destroy(KDTreeNode* curNode);
    void copy(const KDTree<Dim>& copyTrees);
    void copyTreeNodes(KDTreeNode* curNode, KDTreeNode* other);
    void FNNHelper(KDTreeNode* curNode, const Point<Dim>& query, Point<Dim>& currentBest, int curDim, double& bestRadius, bool& firstRun) const;
    double calculateDistance(const Point<Dim>& query ,Point<Dim>& currentBest) const;
